...
void add_to_sent_packet(NETSIM_ID dev,NetSim_PACKET* packet)
{
	...
	if(packet->nSourceId == dev || packet->nDestinationId==dev || packet->nControlDataType%100 == MAC_PROTOCOL_IEEE802_11)
		return;
	if(!p)
...
void add_watchdog_timer(NETSIM_ID dev,NetSim_PACKET* packet)
{
	...
	if(packet->nDestinationId == packet->nReceiverId)
		return;
...
bool check_in_sent_list(NETSIM_ID dev,NetSim_PACKET* packet)
{
	...
	while(temp)
	{
		if(temp->nSourceId == packet->nSourceId &&
			temp->nDestinationId == packet->nDestinationId &&
